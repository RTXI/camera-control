...
camera-control::camera-control(void) : DefaultGUIModel("camera-control",::vars,::num_vars)
{
  duration = 0.040;
  NumFrames = 1024;
  FrameRate = 25;
...
camera-control::~camera-control(void) {}

// execute is run every time step
void camera-control::execute(void) {
  ...
  if(input(0)==1)
    {
      if(count<0 || count>(OffFrameStartCount+CameraDurationCount)) //Do not start a new acquisition if the old one is not finished.
	{
	  count = 0;
	  FrameCount++;
	}
...
void camera-control::update(DefaultGUIModel::update_flags_t flag) {
  switch(flag) {
  case INIT:
  ...
  printf("camera-control Settings: durationCnt=%d, LampDelayCnt=%d, CameraDelayCnt=%d, CameraDurationCt=%d\n", durationCount, LampDelayCount, CameraDelayCount, CameraDurationCount);
